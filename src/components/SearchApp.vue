...
<template>
  <section class="App-search">
    <label class="label-search">
      {{ $t('search_component.search_label') }}
      <svg class="icon-search" focusable="false" xmlns="http://www.w3.org/2000/svg" viewBox="0 0 24 24"><path d="M15.5 14h-.79l-.28-.27A6.471 6.471 0 0 0 16 9.5 6.5 6.5 0 1 0 9.5 16c1.61 0 3.09-.59 4.23-1.57l.27.28v.79l5 4.99L20.49 19l-4.99-5zm-6 0C7.01 14 5 11.99 5 9.5S7.01 5 9.5 5 14 7.01 14 9.5 11.99 14 9.5 14z"></path></svg>
      <input class="App-search-input" type="text" :placeholder="$t('search_component.search_label')" :value="search" @input="searchUpdate">
    </label>
    <button type="button" class='btn clean' :class="{'active': search}" @click="searchClean" :aria-label="$t('search_component.label.clean')">
      ...
      <span>{{ $t('search_component.clean') }}</span>
    </button>
    <button type="button" class="btn keyboard" @click="toggleKeyboard()" :aria-label="$t('search_component.label.keyboard')">
      <img src='../assets/keyboard.png' class="icon-keyboard" alt="Icone Teclado" width="19" height="11" />
      <span>{{ $t('search_component.keyboard') }}</span>
    </button>
    <button type="button" class="btn voice" @click="activeVoice()" :aria-label="$t('search_component.label.voice')">
      ...
      <span>{{ $t('search_component.search_voice') }}</span>
    </button>
    <button type="button" class="btn search" @click="searchGoogle()" :aria-label="$t('search_component.label.search')">
      {{ $t('search_component.search') }}
    </button>
    <button type="button" class="btn lucky" :aria-label="$t('search_component.label.lucky')">
      {{ $t('search_component.lucky') }}
    </button>
  </section>
</template>
