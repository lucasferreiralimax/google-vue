...
<template>
  <section class="App-search">
    <label class="label-search">
      ...
      <input class="App-search-input" type="text" placeholder="Pesquise no Google ou digite um URL" :value="search" @input="searchUpdate">
    </label>
    <button type="button" class='btn clean' :class="{'active': search}" @click="searchClean" aria-label="Botão limpar campo de busca">
    ...
    <button type="button" class="btn keyboard" @click="toggleKeyboard()" aria-label="Botão habilitar teclado virtual">
      <img src='../assets/keyboard.png' class="icon-keyboard" alt="Icone Teclado" width="19" height="11" />
      <span>Teclado virtual</span>
    </button>
    <button type="button" class="btn voice" @click="activeVoice()" aria-label="Botão habilitar reconhecimento de voz">
    ...
    <button type="button" class="btn search" @click="searchGoogle()" aria-label="Botão pesquisar">
      Pesquisa Google
    </button>
    <button type="button" class="btn lucky" aria-label="Botão pesquisar aleatório com sorte">
      Estou com sorte
    </button>
  </section>
</template>
